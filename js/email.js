...
function sendRequest() {
	...
	try {
		...
		// var customWindow = window.open('thank-you.php', '_blank', '');
		// setTimeout(() => { customWindow.close(); }, 3000);
		// setTimeout(() => { customWindow.close(); }, 4000);
		...
		// 	});
		// }
		// var customWindow = await sleep();

		function showNews() {
			news = window.open("", "NewsWindow", "width=900, height=700");
			news.document.write("<p>Message sent successfully, back to previous page in 3 seconds</p>");
			window.open('https://www.vapeoemzmarfa.com/thank-you', "NewsWindow");
			setTimeout(function(){news.close()}, 3000);
		}
		showNews();
